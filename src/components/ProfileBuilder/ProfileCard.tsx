...
const ProfileCard = ({
  ...
  return (
    <Card 
      ref={cardRef}
      data-profile-card
      className={`questionnaire-card group p-4 md:p-7 lg:p-6 
        min-h-[240px] md:min-h-[320px] lg:min-h-[280px]
        hover:scale-[1.02] transition-transform duration-300 ${
        isMobile ? 'active:scale-[0.98] touch-action-manipulation' : ''
      }`}
      onTouchStart={isMobile ? handleCardTouch : undefined}
    >
      <div className="space-y-2 md:space-y-4 lg:space-y-3 h-full flex flex-col">
        ...
        <div className="bg-white/10 rounded-lg p-3 md:p-4 lg:p-4 border border-white/20 ring-1 ring-white/10 shadow-inner flex-1">
          <ul className="space-y-1.5 md:space-y-2.5 lg:space-y-2 text-pink-200/80 text-xs md:text-sm lg:text-base font-normal leading-relaxed">
