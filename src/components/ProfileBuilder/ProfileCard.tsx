...
const ProfileCard = ({
  ...
  return (
    <Card 
      ref={cardRef}
      data-profile-card
      className={`questionnaire-card group p-3 md:p-7 lg:p-7 
        min-h-[200px] md:min-h-[320px] lg:min-h-[320px]
        hover:scale-[1.02] transition-transform duration-300 ${
        isMobile ? 'active:scale-[0.98] touch-action-manipulation' : ''
      }`}
      onTouchStart={isMobile ? handleCardTouch : undefined}
    >
      <div className="space-y-1 md:space-y-4 lg:space-y-4 h-full flex flex-col">
        ...
        <div className="bg-white/10 rounded-lg p-2 md:p-4 lg:p-4 border border-white/20 ring-1 ring-white/10 shadow-inner flex-1">
          <ul className="space-y-1 md:space-y-2.5 lg:space-y-2.5 text-pink-200/80 text-xs md:text-sm lg:text-base font-normal leading-relaxed">
