...
const ChatContainer = forwardRef<ChatContainerRef, ChatContainerProps>(({ 
  ...
  const renderMessages = () => (
    <>
      ...
      <div className={`${isMobile ? 'px-4' : 'md:max-w-3xl lg:max-w-4xl md:mx-auto md:px-6'}`}>
        <div 
          className={`flex ${isMobile ? 'gap-1.5' : 'gap-3'} items-end justify-end transition-opacity duration-200 ${isMobile ? 'mb-3' : 'mb-2 md:mb-3'} ${userTyping ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} 
          aria-live="polite"
    ...
    <div className="flex-1 min-h-0 md:max-h-none relative bg-burgundy-950">
      {isMobilePhone ? (
        ...
        <div 
          ...
          style={{ 
            top: 'calc(env(safe-area-inset-top) + 6rem)', // Optimized spacing for nav bar + ChatHeader
