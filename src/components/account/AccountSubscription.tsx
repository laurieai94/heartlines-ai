...
const AccountSubscription = () => {
  ...
  return <div className={`${isMobile ? '' : 'space-y-2.5'} touch-manipulation`}>
      ...
      <Card className={`bg-white/10 backdrop-blur-sm border border-white/20 ${isMobile ? 'rounded-lg' : ''}`}>
        <CardHeader className={isMobile ? 'p-1.5' : 'p-2.5'}>
          <CardTitle className={`text-white flex items-center gap-1.5 ${isMobile ? 'text-sm' : 'text-base'}`}>
        ...
        }} disabled={managing} className={`questionnaire-button-secondary touch-manipulation ${isMobile ? 'text-[13px] py-1 h-7' : 'text-sm py-1.5'}`}>
          ...
          {plans.map(plan => <Card key={plan.name} className={`relative bg-white/10 backdrop-blur-sm border transition-all duration-300 ${plan.current ? 'border-pink-400/50 bg-white/15' : 'border-white/20 hover:border-white/30'} ${isMobile ? 'plan-card' : ''}`}>
              ...
              <CardHeader className={`text-center ${isMobile ? 'p-1.5' : 'p-2.5'}`}>
                ...
                <div className={`font-bold text-white ${isMobile ? 'text-base' : 'text-xl'}`}>
                  {plan.price}
                  <span className={`font-normal text-white/60 ${isMobile ? 'text-xs' : 'text-sm'}`}>/month</span>
                </div>
              ...
              <CardContent className={`${isMobile ? 'p-1.5 pt-0 space-y-1' : 'p-2.5 pt-0 space-y-1.5'}`}>
                ...
                {plan.current ? <Button disabled className={`w-full bg-green-500/20 text-green-400 border border-green-400/30 touch-manipulation ${isMobile ? 'text-[13px] py-1 h-7 mt-2' : 'text-sm py-1.5 mt-3'}`}>
            ...
            }} disabled={upgrading === plan.tier} className={`w-full questionnaire-button-primary touch-manipulation touch-feedback ${isMobile ? 'text-[13px] py-1 h-7 mt-2' : 'text-sm py-1.5 mt-3'}`}>
                    <Zap className={`mr-1 ${isMobile ? 'h-2.5 w-2.5' : 'h-3 w-3'}`} />
                    {upgrading === plan.tier ? 'Processing...' : `Upgrade to ${plan.name}`}
                  </Button> : <Button disabled variant="outline" className={`w-full bg-white/5 border-white/20 text-white/60 touch-manipulation ${isMobile ? 'text-[13px] py-1 h-7 mt-2' : 'text-sm py-1.5 mt-3'}`}>
                    Current Plan
                  </Button>}
              </CardContent>
            </Card>)}
      ...
      {usagePercentage > 80 && !subscribed && <Card className={`bg-orange-500/20 border border-orange-400/30 ${isMobile ? 'rounded-lg' : ''}`}>
          <CardContent className={isMobile ? 'p-1.5' : 'pt-5'}>
            <div className={`flex items-start ${isMobile ? 'gap-1.5' : 'gap-3'}`}>
              <div className={`bg-orange-400 rounded-full animate-pulse flex-shrink-0 ${isMobile ? 'h-1.5 w-1.5 mt-0.5' : 'h-2 w-2 mt-1'}`} />
              <div className="min-w-0">
                <h4 className={`font-medium text-orange-200 ${isMobile ? 'text-sm' : 'text-sm'}`}>Usage Alert</h4>
                <p className={`text-orange-300/80 ${isMobile ? 'text-xs leading-tight' : 'text-sm'}`}>
