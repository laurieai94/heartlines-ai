...
import { ArrowLeft, ArrowRight, Heart, Lock, Unlock, Sparkles } from "lucide-react";
...
const QuestionnaireFooter = ({ 
  ...
  const getUnlockPreview = () => {
    ...
    return (
      <div className="text-center mt-2 p-3 bg-white/5 backdrop-blur-sm rounded-lg border border-white/10">
        <div className="flex items-center justify-center gap-2 mb-2">
          <Lock className="w-4 h-4 text-orange-400" />
          <span className="text-white/90 font-medium text-sm">What you'll unlock:</span>
        </div>
        <div className="text-xs text-white/70 space-y-1">
          <div>• Personalized AI coaching tailored to YOUR situation</div>
          <div>• Real-time relationship insights and advice</div>
          ...
          {currentSection === 4 && (
            <div className="text-center">
              <Button
                ...
                } text-white flex items-center gap-2 px-6 py-3 text-sm rounded-lg shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200 disabled:opacity-50 disabled:hover:scale-100 font-semibold`}
              >
                {overallProgress === 100 ? (
                  <>
                    <Unlock className="w-4 h-4" />
                    Start Using Kai AI Coach!
                  </>
                ) : (
                  <>
              ...
              {overallProgress === 100 && (
                <p className="text-emerald-300 text-xs mt-2 font-medium">
                  🎉 Your personalized AI relationship coach is ready!
                </p>
              )}
            </div>
          )}
