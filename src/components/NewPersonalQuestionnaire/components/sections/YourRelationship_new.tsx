    ...
    <div className={`space-y-4 transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-60'}`} data-section="2">
      ...
      {profileData.relationshipStatus && (
        <SimpleContinueButton 
          onClick={() => {
            if (isSingle) scrollToQuestion('question-dating-challenges');
            else if (isTalking) scrollToQuestion('question-talking-duration');
            else if (hasRelationship) scrollToQuestion('question-relationship-length');
            else if (isSeparatedDivorced) scrollToQuestion('question-separation-situation');
          }} 
        />
      )}
