    ...
    <div className={`flex gap-2 md:gap-3 items-center px-3 md:px-0 touch-action-manipulation pointer-events-auto cursor-text ${readOnly ? 'group' : ''}`} style={{ minHeight: (readOnly && window.innerWidth < 768) ? '52px' : '44px' }}>
      <div className={`flex-1 relative isolate rounded-2xl overflow-hidden ${
        readOnly 
          ...
          : 'bg-burgundy-900/40 backdrop-blur-xl border-2 border-white/35 ring-2 ring-white/15 shadow-[0_4px_24px_rgba(0,0,0,0.4)] focus-within:border-coral-300/50 focus-within:ring-coral-400/20 focus-within:shadow-[0_8px_32px_rgba(0,0,0,0.5)] focus-within:shadow-coral-400/20 transition-all duration-300'
      }`}>
        <Textarea
          unstyled
          ref={textareaRef}
