...
import { User, MessageCircle, Lightbulb, Heart, Home } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 sticky top-0 z-30">
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        <nav role="navigation" aria-label="Primary" className={`flex justify-center ${compact ? 'pb-2' : 'pb-4'}`}>
          <TabsList 
            className={`w-full max-w-4xl ${compact ? 'h-10' : 'h-12'} flex md:grid md:grid-cols-5 overflow-x-auto no-scrollbar rounded-full p-1 gap-1 bg-background/60 backdrop-blur-md border border-border/60 shadow-sm`}
          >
            <TabsTrigger 
              value="home" 
              title="Home"
