...
const ChatRow = memo(({ message, userAvatar, userName }: {
  ...
  <div className={`flex gap-2 items-end ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
    ...
    {message.type === 'user' && (
      <div className="w-6 h-6 md:w-8 md:h-8 lg:w-9 lg:h-9 aspect-square rounded-full overflow-hidden ring-2 ring-coral-400/40 flex-shrink-0">
        <img
          src={userAvatar}
          alt={userName}
...
const HeroPhoneScroll: React.FC<HeroPhoneScrollProps> = ({ className = '', style }) => {
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`} style={style}>
      ...
      <div className="relative flex items-start justify-center z-20 pt-4 pb-0 px-2 sm:p-2 lg:p-4">
