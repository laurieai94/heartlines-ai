        ...
        <div className="mt-1 space-y-4 md:space-y-5 lg:space-y-6 pb-2">
          ...
          <Card className="backdrop-blur-xl rounded-2xl p-2.5 md:p-3 bg-gradient-to-br from-white/35 via-white/30 to-white/25 border border-white/50 shadow-xl w-full box-border">
            ...
            <div className="mt-1 w-full bg-white/25 rounded-full h-1.5">
              <div className="bg-gradient-to-r from-coral-400 to-pink-500 h-1.5 rounded-full transition-all" style={{
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-2.5 md:gap-2.5 lg:gap-3 w-full mx-auto items-stretch overflow-x-hidden mt-8 md:mt-9">
            ...
            return <Card key={plan.id} className={`backdrop-blur-xl bg-gradient-to-br from-white/35 via-white/30 to-white/25 relative rounded-3xl shadow-2xl border border-white/50 transition-all duration-300 will-change-transform md:hover:-translate-y-2 flex flex-col w-full box-border ${getOrderClass()} ${isPopular ? 'ring-2 ring-coral-400/60 border-coral-400/40' : ''} ${isRecommended && !isPopular ? 'ring-2 ring-coral-400/60 border-coral-400/40' : ''}`}>
                  ...
                  <div className="p-3 md:p-3.5 flex-1 flex flex-col">
                    ...
                    {/* CTA Button */}
                    <Button onClick={() => handleUpgrade(plan.tier as 'glow' | 'vibe' | 'unlimited')} disabled={false} className={`w-full rounded-full text-sm font-semibold mt-auto ${isPopular || isRecommended ? 'bg-gradient-to-r from-coral-400 to-pink-500 text-white hover:from-coral-500 hover:to-pink-600 shadow-lg' : 'bg-white/15 text-white hover:bg-white/25 border border-white/30'}`}>
                  {isCurrent ? 'current plan' : !canUpgrade ? 'not available' : plan.buttonText}
                    </Button>
                  </div>
                </Card>;
          })}
