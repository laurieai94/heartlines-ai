-- Re-run idempotent migration with corrected cron schedule quoting
-- 1) Table changes (idempotent)
...
-- 4) Schedule the email processor (idempotent) with proper nested dollar-quoting
DO $$
BEGIN
  IF NOT EXISTS (
